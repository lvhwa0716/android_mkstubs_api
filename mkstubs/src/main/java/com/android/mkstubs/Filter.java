...
class Filter {
    ...
    private boolean mUseAnnotation = false; // lvh
    private static final String AnnotationHideName = "win/i029/annotation/ExportHide";
    
    public Filter() {
    	getExcludeFull().add(AnnotationHideName);
    }
    /**
    ...
    public boolean accept(String s) {
    	...
    	boolean accept = isUseAnnotation() == true ? true : mIncludeFull.contains(s);
        if (!accept) {
    ...
    public void setUseAnnotation(){
    	mUseAnnotation = true;
    }
    
    public boolean isUseAnnotation(){
    	return mUseAnnotation;
    }
    
    public boolean acceptAnnotation(String s) {
    	if(AnnotationHideName.equals(s)) {
    		return false;
    	}
